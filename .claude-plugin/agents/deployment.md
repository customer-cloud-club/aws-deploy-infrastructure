...
## AWS MCP Integration (2025-11-19) 🆕

### AWS ECS Deployment Support

DeploymentAgentは、FirebaseだけでなくAWS ECS deploymentもサポートします。

**Required MCP Servers**:
- `aws-ecs` - ECS service management
- `aws-cloudwatch` - Log analysis, metrics monitoring
- `aws-cfn` - CloudFormation stack validation
...
aws_deployment_flow:
  1_pre_validation:
    - aws-cfn → describe_stacks(stack_name="ApplicationStack-development")
    - Check stack status = UPDATE_COMPLETE
  ...
  4_ecs_deployment:
    - aws-ecs → update_task_definition(new_image_tag)
    - aws-ecs → update_service(service="aifactory-development-api-service")
    - aws-ecs → monitor_deployment_status()

  5_health_verification:
    - aws-cloudwatch → analyze_log_group(log_group="/ecs/aifactory-development-api")
    - aws-cloudwatch → get_metric_data(metric="TargetResponseTime")
    - Check: No errors in last 5 minutes
    - Check: Response time < 1s

  6_rollback_on_failure:
    - aws-cloudwatch → analyze_log_group(filter="ERROR")
    - aws-ecs → get_task_failure_reasons()
    - aws-ecs → rollback_to_previous_task_definition()
...
```bash
aws-cfn → describe_stacks(stack_name="ApplicationStack-development")
# Returns: stack status, resource health
```

**ECS deployment**:
```bash
aws-ecs → update_task_definition(
  family="aifactory-development-api",
  container_definitions=[{
    name: "api",
    image: "607520774686.dkr.ecr.ap-northeast-1.amazonaws.com/aifactory-api:v1.2.3"
  }]
)

aws-ecs → update_service(
  cluster="aifactory-development-cluster",
  service="aifactory-development-api-service",
  task_definition="aifactory-development-api:123"
)

aws-ecs → get_deployment_status(
  cluster="aifactory-development-cluster",
  service="aifactory-development-api-service"
)
```
...
aws-cloudwatch → analyze_log_group(
  log_group="/ecs/aifactory-development-api",
...
aws-cloudwatch → get_metric_data(
  ...
  dimensions={"LoadBalancer": "aifactory-development-alb"},
  time_range="last 5 minutes",
  statistic="Average"
)
```
...
- Health check via CloudWatch + ALB metrics

### Error Handling

```yaml
deployment_failure_handling:
  scenario_1_ecs_task_fails:
    - aws-ecs → get_task_stopped_reason()
    - aws-cloudwatch → analyze_log_group(filter="ERROR")
    - Identify: Image pull error, health check failure, etc.
    - Action: Rollback + Report

  scenario_2_health_check_fails:
    - aws-cloudwatch → analyze_log_group(filter="5xx OR ERROR")
    - aws-cloudwatch → get_metric_data(metric="UnHealthyHostCount")
    - Identify: Application error, database connection failure
    - Action: Rollback + Escalate to TechLead

  scenario_3_deployment_timeout:
    - aws-ecs → get_deployment_status()
    - Check: Deployment stuck in RUNNING state > 10 minutes
    - Action: Cancel deployment + Rollback
```

### Deployment Commands

**Firebase deployment (existing)**:
```bash
npm run deploy:staging
npm run deploy:production
```

**AWS ECS deployment (new)**:
```bash
# Deploy backend API to ECS
npm run deploy:ecs:development
npm run deploy:ecs:production

# Via DeploymentAgent
npm run agents:deploy -- --target ecs --environment development
```

### Metrics Collection

AWS deployments collect enhanced metrics:

```yaml
aws_deployment_metrics:
  deployment_id: "deploy-1759552488828"
  ...
  ecr_push_duration_ms: 30000
  ecs_deployment_duration_ms: 180000
  task_startup_duration_ms: 60000

  # Health check
  health_check_duration_ms: 15000
  health_check_attempts: 3
  cloudwatch_errors_found: 0
  alb_response_time_avg_ms: 250

  # Status
  status: "success"
  deployment_url: "https://aidreams-factory.com"
  ecs_service_arn: "arn:aws:ecs:ap-northeast-1:..."
```

### Integration Documentation

See: [Agent AWS MCP Integration Guide](../../.ai/AGENT_AWS_MCP_INTEGRATION.md)
